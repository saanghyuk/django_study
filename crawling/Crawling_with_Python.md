...
| soup.find_all() | 선택자 만족하는 전체를 가져다 줌(참고로 limit=2 등의 파라미터를 주면, 순서대로 딱 그만큼만 가져다 줌. 아니면 싹다 리스트로 가져옴). link2 = soup.find_all('a', class_="syster") 이렇게 조건도 줄 수 있음. string=["Elsie", 'Title'])id="link2", string="title" 이런식으로 조건별로 찾을 수 있음. |
